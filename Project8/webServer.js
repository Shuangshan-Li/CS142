...
app.get("/user/mentionOptions", function (request, response) {
  ...
  User.find({}).exec((e, users) => {
    ...
    async.forEachOf(
      ...
      function (user, i, callback) {
        let {_id, first_name, last_name} = user;
        result[i] = {id: _id, display: `${first_name} ${last_name}`};
        callback();
      },
      (e) => {
        if (e) {
    ...
    async.forEachOf(
      ...
      (e) => {
        if (e) {
          ...
          response.status(200).send(
            result.sort((a, b) => {
              if (b.liked_by.length - a.liked_by.length !== 0) {
...
app.get("/photo/:photo_id", function (request, response) {
  if (!request.session.user_id) {
    response.status(401).send("you should log in first!");
    return;
  }
  ...
  Photo.findOne({_id: photo_id}).exec((err, photo) => {
    ...
    User.findOne({_id: photo.user_id}).exec((err, photo_owner) => {
      let result = {
        _id: photo_id,
...
app.post("/commentsOfPhoto/:photo_id", function (request, response) {
  ...
  let new_mentions = request.body.mentionsToAdd;
  if (!new_comment) {
    ...
    async.each(
      ...
      function (user_id, callback) {
        User.findOne({_id: user_id}).exec((err, user) => {
          if (err) {
            response.status(400).send("invalid user_id!");
            return;
          }
      ...
      function (err) {
        if (err) {
          response.status(400).send("failed to update mentions");
          return;
        }
        response.status(200).send();
      }
...
app.post("/photos/new", function (request, response) {
  if (!request.session.user_id) {
    ...
    processFormBody(request, response, function (err) {
      ...
      let permissions = JSON.parse(request.body.usersPermissed);
      let visible = JSON.parse(request.body.visible);
      ...
      fs.writeFile("./images/" + filename, request.file.buffer, function (err) {
        ...
        let permitted = Object.entries(permissions)
          .filter((k) => k[1])
          .map((k) => k[0]);
        permitted.push(request.session.user_id);
        Photo.create(
          {
            file_name: filename,
            date_time: time,
...
app.post("/photoLikedToggle/:photo_id", function (request, response) {
  if (!request.session.user_id) {
    response.status(401).send("you should log in first!");
    return;
  }
  ...
  let liked = JSON.parse(request.body.liked);
  ...
  Photo.findOne({_id: photo_id}).exec((err, photo) => {
    ...
    let user_index = photo.liked_by.indexOf(user_id);

    if (liked) {
      if (user_index > -1) {
        response
          .status(400)
          .send("invalid operation! you already liked this photo!");
        return;
      }
      ...
      if (user_index < 0) {
        response
          .status(400)
          .send("iinvalid operation! you didn't like this photo!");
        return;
      }
      photo.liked_by.splice(user_index, 1);
    }
    photo.save();
    response.status(200).send();
  });
...
app.get("/fetchFavorites", function (request, response) {
  if (!request.session.user_id) {
    response.status(401).send("you should log in first!");
    return;
  }
  User.findOne({_id: request.session.user_id}).exec((err, user) => {
    if (err) {
      response.status(400).send("invalid user_id!");
      return;
    }
    ...
    async.each(
      ...
      (photo_id, callback) => {
        Photo.findOne({_id: photo_id}).exec((err, photo) => {
          if (err) {
            response
              .status(400)
              .send("invalid photo_id when fetching favorites!");
            return;
          }
          result.push({
            file_name: photo.file_name,
      ...
      function (err) {
        if (err) {
          response.status(400).send("failed to fetch favorites!");
          return;
        }
        response.status(200).send(result);
      }
...
app.get("/deleteFavorite/:photo_id", function (request, response) {
  ...
  User.findOne({_id: request.session.user_id}).exec((err, user) => {
    ...
    user.favorites.splice(user.favorites.indexOf(request.params.photo_id), 1);
    user.save();
    response.status(200).send();
  });
});
